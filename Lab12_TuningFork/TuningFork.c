...
void SysTick_Handler(void){
	...
	SW = ((GPIO_PORTA_DATA_R & 0x08)>>3); // Reads current Switch PA3 and moves to BIT 0;
	 ...
	 // Other combinations mean switch is either held or off 
	if (SW==1 && PREV_SW==0) {
		SOUND = !SOUND; // switches on/off
	}
	if (SOUND) {
		GPIO_PORTA_DATA_R ^= 0x04; // toggles PA3 on/off and outputs sound
	} else {
		GPIO_PORTA_DATA_R &= ~0x04;// PA3 = off , sound off
	}
