...
int main(void){  
  ...
  while(1){
		Current_Data = GPIO_PORTF_DATA_R &0x13;
		toggle = 0;
		SW1 = GPIO_PORTF_DATA_R&0x10;     // read PF4 into SW1
    SW2 = GPIO_PORTF_DATA_R&0x01;     // read PF0 into SW2
		//negative logic for SW1 & SW2
		 if(SW1&&SW2){                     // neither is pressed
         GPIO_PORTF_DATA_R &= ~0x02;    // LED is off
     } else{                           
				Led = GPIO_PORTF_DATA_R;		
				Led = Led ^0x02;            // TOGGLE LED
				GPIO_PORTF_DATA_R = Led;  
				Delay();
    }
		if (Current_Data != (GPIO_PORTF_DATA_R &0x13)) //Dump when data changes
			if(i<50){
				now = NVIC_ST_CURRENT_R;
				Time[i] = (last-now)&0x00FFFFFF;  // 24-bit time difference
				Data[i] = GPIO_PORTF_DATA_R; // Record PORTF_DATA
				last = now;
				i++;
				}
	}
