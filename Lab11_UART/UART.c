...
// as part of Lab 11 implement this function
  unsigned long i;
	if (n==0){ // This takes care of the while loop if n is 0
		String[3] = '0';
	}
	if (n>9999) { // prints an error format
		for (i=0;i<4;i++){
			String[i] = '*';
		}
	} else {
		...
		i=3;
		while (n>0) {
		// Populate String backwards from String[3] to String[0]
		String[i] = n%10 + 0x30;// add the ASCII offest of 0x30
		n = n/10;
		i--;
		}
	}
...
// as part of Lab 11 implement this function
  unsigned long i;
	if (n > 9999) { //prints an error format
		...
		for (i=0;i<5;i++) { 
			String[i] = '0'; // Update String with Zeroes
		}
		i=4;
		while (n>0) {
			// Populate String backwards from String[4] to String[1]
			String[i] = n%10 + 0x30; // add the ASCII offest of 0x30
			n = n/10;
			i--;
		}
	}
	String[0] = String[1]; // Most significant in first position, to String[0]
	String[1] = '.';			 // Add the '.' after most significant
  String[5] = ' ';			// Adds the end 'cm'
	String[6] = 'c';			 
	String[7] = 'm';
	String[8] = '\0';		// Null terminated
}
